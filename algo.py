...
from duo_efficient_curriculum import sample_tempered_softmax_topk, sample_topk_gaussian
import trainer_base
import utils
...
class DUO(DUO_BASE):
  def __init__(self, config, tokenizer):
    ...
    self.gumbel_tau_log10_start = self.config.algo.curriculum.gumbel_tau_log10_start
    self.gumbel_tau_log10_end = self.config.algo.curriculum.gumbel_tau_log10_end
  ...
  def _init_curriculum_series(self):
    ...
    self.register_buffer('coefficients_m', m, persistent=False)
    self.register_buffer('coefficients_i', i, persistent=False)
  ...
  def _init_curriculum_approx(self):
    ...
    if self.config.algo.curriculum.mode == 'sigmoid':
      fn = utils.duo_to_alpha_dalpha_sigmoid
    elif self.config.algo.curriculum.mode == 'sigmoid-edge-corrected':
      fn = utils.duo_t_to_alpha_dalpha_sigm_corrected
    elif self.config.algo.curriculum.mode in ('poly3', 
      'poly5', 'poly7', 'poly9'):
      fn = utils.duo_to_alpha_dalpha_poly
    else:
      raise ValueError(self.config.algo.curriculum.mode)
    fn = torch.compile(fn)
    self._t_to_alpha_dalpha_compiled = lambda t: fn(t, *coefficients)
  ...
  def _gamma_to_alpha_dalpha_cached(self, gamma_t):
    ...
    usdm_alpha_t = DUO._gamma_to_alphat(self, gamma_t)
    T = 1000
    usdm_dalpha_t = gamma_t_prime * T * (
      DUO._gamma_to_alphat(self, gamma_t + 1 / T) - usdm_alpha_t)
  ...
  def nll(self, x0, output_tokens,
    ...
    if self.config.algo.curriculum.mode == 'simple':
      ...
      softmax_approx, topk_indices, xt_usdm = sample_tempered_softmax_topk(
        extra_index=x0,
